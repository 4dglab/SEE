...
map_info = pd.read_csv(
    os.path.join(root_dir, "GSE223917", "map_result.csv"),
    sep="\t",
    index_col=0,
)
# hic
cools_path = os.path.join(root_dir, "GSE223917", "cool")
hic = anndata.read_h5ad(os.path.join(root_dir, "GSE223917", "hic_result.h5ad"))
# rna
rna = anndata.read_h5ad(os.path.join(root_dir, "GSE223917", "rna_result.h5ad"))
rna_marker_gene = pd.read_csv(
    os.path.join(root_dir, "GSE223917", "rna_marker_gene.csv"),
    sep="\t",
    index_col=0,
)
...
for cell_type, row in cell_type_count.iterrows():
    for i in range(min(cell_type_count.max()[0] - row["scHiC"], int(row["scHiC"] / 2))):
        ...
        best_rna, best_hic = find_best_RNA(_case.name, cell_type), find_best_HiC(
            _case["scHiC"], cell_type
        )
