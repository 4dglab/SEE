...
def catch_matrix(_c, chrom: str, start: int, end: int, strand: str):
    ...
    if strand == '-':
        _start, _end = start - length, start + length
    else:
        _start, _end = end - length, end + length

    mat = _c.matrix(balance=False).fetch('{}:{}-{}'.format(chrom, _start, _end))
    return mat
...
np.save(os.path.join(root_dir, 'sclab', 'terminator', 'train_dataset.npy'), train_dataset)
np.save(os.path.join(root_dir, 'sclab', 'terminator', 'eval_dataset.npy'), eval_dataset)