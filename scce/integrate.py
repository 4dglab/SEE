...
class DataTool:
    ...
    def rna_highly_variable_genes(self):
        ...
        _index = set(self.rna.var[self.rna.var['highly_variable']==True].index) & set(marker_genes_index)
...
def interval_dist(x: Interval, y: Interval, resolution: int) -> int:
    ...
    x_range = set([i for i in range(int(x.start/resolution)*resolution, int(x.end/resolution)*resolution+resolution, resolution)])
